(function () {
	...
	angular
		...
		.controller('SV.02svServPlnListCtrl', ['$scope', '$state', '$timeout', 'SvListPageVO', 'SV.01svServRecListBiz', 'resData', 'Page',
			function ($scope, $state, $timeout, SvListPageVO, SvServPlnListBiz, resData, Page) {
				var page  = $scope.page = new Page({ auth: resData.access });

			var page_title = "서비스계획정보";
			var objBiz = SvServPlnListBiz;
			var strGridProcedureParam = "USP_SV_PLNSEARCH_GET&" +
										...
										"NM_EQM@s|DC_SRN@s|NM_MODEL@s|NM_PJT@s|TP_PER@s|STRT_PER@s|END_PER@s";
			var searchObj_perTpCdList = [
											{ CD: '1', NAME: '접수일자' },
											{ CD: '2', NAME: '완료요청일자' }
										];
			var gridObj_BoxTitle = "서비스계획정보";
			var gridObj_rowTemplate = "<div ng-dblclick=\"grid.appScope.vm.gridObj.moveDetailPage(row.entity)\" " +
					                  ...
					                  "</div>";
			var gridObj_InsertPage = "/02svServPln/insert";
			var gridObj_DetailPage = "/02svServPln/detail?ids=";
			
			var gridObj_GridField = [
				{ visible: true, field: 'NO_SERV_REC'      , width: 130 , cellClass: 'ta-c', displayName: '접수번호'},
				{ visible: true, field: 'NM_RECR_EMP'      , width: 80  , cellClass: 'ta-c', displayName: '접수자'},
			...
			var vm = {
				...
				gridObj : {
					...
					moveInsPage : function () {
						$state.go('app.svServPln', { kind: 'insert', menu: null, ids: null });
					},
					/**
					 * 상세페이지로 이동한다
					 * @param {object} row
					...
					moveDetailPage : function (row) {
						$state.go('app.svServPln', { kind: 'detail', menu: null, ids: row.NO_SERV_REC });
					},
					/**
					 * 엑셀파일을 다운로드한다.
					 */
