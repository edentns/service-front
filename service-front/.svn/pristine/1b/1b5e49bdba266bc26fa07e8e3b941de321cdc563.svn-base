(function() {
    ...
    angular
        ...
        .controller('CoSalesPrscCtrl', CoSalesPrscCtrl);


    CoSalesPrscCtrl.$inject = ['$scope', 'CO.dashboardSvc', 'CoDashboardBiz', '$timeout'];

    function CoSalesPrscCtrl($scope, CoDashboardSvc, CoDashboardBiz, $timeout) {
        $scope.$on('dashboard:query', function(event, payload) {
            CoDashboardBiz.setPayload(salesPrsc, payload);
        ...
        var salesPrsc = $scope.salesPrsc = {
            ...
            find: function() {
                ...
                return CoDashboardSvc
                    .getSales(me.getFindParam(target.search))
            ...
            config: {
                ...
                target  : {
                    y1: [
                        ...
                        { visible: false,   field: "FORECAST",          name: "FORECAST" }, { visible: true,    field: "COMMIT",            name: "COMMIT" }
                    ],
                    y2: [
        ...
        salesPrsc.chart.first.config.setting = {
            padding: {left: 60},
            axis: [
                {
                    data: salesPrsc.chart.first.config.data,
            ...
            brush: [
                { type: "column",   target: salesPrsc.chart.first.config.target.y1, outerPadding: 10, innerPadding: 3, axis: 0, colors: [0, 1, 2, 3], animate: true },
                { type: "line",     target: salesPrsc.chart.first.config.target.y2, axis: 1, colors: [4, 5], animate: true, display: "all" },
                { type: "scatter",  target: salesPrsc.chart.first.config.target.y2, size : 10, axis : 1, colors: [4, 5] }
            ],
            widget: [
        ...
        salesPrsc.chart.second.config.setting = {
            padding : { left : 60 },
            axis 	: [
                {
                    data    : salesPrsc.chart.second.config.data,
                    x 		: { type : "block", domain : "DEPT_NAME" },
                    y 		: {
                        type 	: "range",
                        domain  : function (data) {
            ...
            brush : [
                { type: "column",   target: salesPrsc.chart.second.config.target.y1, outerPadding: 10, innerPadding: 3, axis: 0, colors: [0, 1], animate: true },
                { type: "line",     target: salesPrsc.chart.second.config.target.y2, axis: 1, colors: [2, 3], animate: true, display: "all" },
                { type: "scatter",  target: salesPrsc.chart.second.config.target.y2, size: 10, axis: 1, colors: [2, 3] }
            ],
            widget : [
                { type : "tooltip", brush: [0, 2], format: function (data, key) {
                    return { key: key.field, name: key.name, value: edt.formatPrice(data[key.field]) };
