(function () {
	...
	angular
		...
		.controller('SV.01svServRecListCtrl', ['$scope', '$state', '$timeout', 'SvListPageVO', 'SV.01svServRecListBiz', 'resData', 'Page',
			function ($scope, $state, $timeout, SvListPageVO, SvServRecListBiz, resData, Page) {
				var page  = $scope.page = new Page({ auth: resData.access });

				var page_title = "서비스접수정보";
				var objBiz = SvServRecListBiz;
				var strGridProcedureParam = "USP_SV_RECSEARCH_GET&" +
				...
				var gridObj_rowTemplate = "<div ng-dblclick=\"grid.appScope.vm.gridObj.moveDetailPage(row.entity)\" " +
						                  ...
						                  "</div>";
				var gridObj_InsertPage = "/01svServRec/insert";
				var gridObj_DetailPage = "/01svServRec/detail?ids=";
				...
				var vm = {
					...
					gridObj : {
						...
						moveInsPage : function () {
							$state.go('app.svServRec', { kind: 'insert', menu: null, ids: null });
						},
						/**
						 * 상세페이지로 이동한다
						 * @param {object} row
						...
						moveDetailPage : function (row) {
							$state.go('app.svServRec', { kind: 'detail', menu: null, ids: row.NO_SERV_REC });
						},
						/**
						 * 엑셀파일을 다운로드한다.
						 */
