(function () {
	...
	angular
		.module('SV.04svRecResProc.service')
		.service('SV.04svRecResProcListBiz', ['UtilSvc', 'SvCommonSvc', 'SvListPageVO', 'SV.04svRecResProcSvc', 'SY.codeSvc', 'ModalSvc', 'APP_CODE', '$q', '$timeout',
			function (UtilSvc, SvCommonSvc, SvListPageVO, SvServRecProcSvc, SyCodeSvc, ModalSvc, APP_CODE, $q, $timeout) {
				...
				self.initLoad = function (search, servRecProc) {
					var self = this,
						defer = $q.defer();
					...
					$timeout(function () {
						...
						compositeParam.procedureParam = servRecProc.grid.procedureParam;
						...
						self.getCodeAndData(compositeParam).then(function (res) {
							...
							servRecProc.grid.data = res[9].data.results[0];

							self.setDataAndParam(search, servRecProc);
				...
				self.getCodeAndData = function (param) {
					...
					return $q.all([
						...
						UtilSvc.getList(param)                                   // TODO: 완료리스트를 가져온다.
					]);
				};

				self.setDataAndParam = function (search, servPfm) {
					self._setParam(search);
					self._setGridColumn(servPfm);
				};
				...
				/**
				 * grid column을 설정한다.
				 * @param {object} servRecProc 접수결과처리 Model
				 * @private
				 */
				self._setGridColumn = function (servRecProc) {
					UtilSvc.grid.initColumnSetting(servRecProc);
				};
				...
				/**
				 * Grid Column을 설정한다.
				 * @param {object} servRecProc 접수결과처리
				 */
				self.modalSetColumn = function (servRecProc) {
					var column = servRecProc.column,
						grid   = servRecProc.grid,
