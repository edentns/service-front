...
jui.define("chart.brush.core", [ "jquery", "util.base" ], function($, _) {
	...
	var CoreBrush = function() {
        ...
        function getMinMaxValue(data, target) {
            var seriesList = {},
                targetList = {},
	            isStringFromTarget = true,
	            targetValue;


            for(var i = 0; i < target.length; i++) {
	            isStringFromTarget = _.typeCheck("string", target[i]);
	            targetValue = isStringFromTarget ? target[i] : target[i].field;
                if (!seriesList[targetValue]) {
        ...
        this.getXY = function(isCheckMinMax) {
            ...
            func(function(i, group) {
                ...
                for(var j = 0; j < targetLength ; j++) {
                    var isStringFromTarget = _.typeCheck("string", target[j]),
	                    isVisibleTarget = isStringFromTarget || target[j].visible,
	                    key = isStringFromTarget ? target[j] : target[j].field,
                        value = data[key];
	...
	var ColumnBrush = function() {
	    ...
	    this.draw = function() {
		    ...
		    this.eachData(function(i, data) {
			    ...
			    for (var j = 0; j < this.brush.target.length; j++) {
				    //var value = data[this.brush.target[j]],
				    var isTargetString  =  (typeof this.brush.target[j] === "string"),
					    isTargetVisible = isTargetString || this.brush.target[j].visible,
					    value, tooltipX, tooltipY, position;

				    if (!isTargetVisible) { continue; }

				    value = isTargetString ? data[this.brush.target[j]] : data[this.brush.target[j].field];
...
jui.define("chart.brush.line", [], function() {
	...
	var LineBrush = function() {
        ...
        this.addLineElement = function(elem, idx) {
            if(!this.lineList) {
                this.lineList = [];
            }

	        if (idx) {
		        this.lineList[idx] = elem;
	        } else {
...
jui.define("chart.brush.pie", [ "util.base", "util.math", "util.color" ], function(_, math, ColorUtil) {
	...
	var PieBrush = function() {
        ...
        this.drawText = function(centerX, centerY, centerAngle, outerRadius, value, text) {
			...
			for (var i = 0; i < target.length; i++) {
                ...
                if (this.brush.showText) {
                    var text = this.getFormatText(target[i], value, max),
                        elem = this.drawText(centerX, centerY, centerAngle, outerRadius, value, text);

	                if (elem) { this.addEvent(elem, index, i); }
                    g.append(elem);
                }
...
jui.define("chart.brush.scatter", [ "util.base" ], function(_) {
    ...
    var ScatterBrush = function() {
        ...
        this.getSymbolType = function(key, value) {
	        var isStringFromTarget  = _.typeCheck("string", this.brush.target[key]),
		        isVisibleTarget     = isStringFromTarget || this.brush.target[key].visible,
	            symbol = this.brush.symbol,
                target = isStringFromTarget ? this.brush.target[key] : this.brush.target[key].field;
...
jui.define("chart.widget.tooltip", [ "jquery", "util.base", "util.color" ], function($, _, ColorUtil) {
    ...
    var TooltipWidget = function(chart, axis, widget) {
        ...
        function printTooltip(obj) {
            ...
            } else if(widget.all && _.typeCheck("array", widget.brush) && widget.brush.length>=2) {
	            //if (_.typeCheck("function", widget.custom)) {
		         //   widget.custom();
		         //   setTextInTooltip(["FUNNEL", "FORCAST"]);
	            //}
            } else {
            ...
            function setTextInTooltip(targets) {
                for(var i = 0; i < targets.length; i++) {
                    ...
                    if(msg.name) {
                        texts.get(i).get(0).text(msg.name);
                    } else {
    ...
    var LegendWidget = function(chart, axis, widget) {
	    ...
	    function setLegendStatus(brush) {
		    ...
		    for(var i = 0; i < brush.target.length; i++) {
			    if (_.typeCheck("string", brush.target[i])) {
				    columns[brush.index][brush.target[i]] = true;
			    } else {
				    columns[brush.index][brush.target[i].field] = brush.target[i].visible;
			    }
		    }
	    }
        ...
        function changeTargetOption(brushList) {
            ...
            for(var key in columns[index]) {
	            ...
	            colorsAll.push(colorIndex[key]);
            }

            for(var i = 0; i < brushList.length; i++) {
	            if (_.typeCheck("object", brushList[i].target[0])) {
		            target = brushList[i].target;
		            colors = colorsAll;
	            }
            ...
            chart.emit("legend.filter", [ target, index ]);
        }
	    ...
	    this.getLegendIcon = function(brush) {
		    ...
		    for(var i = 0; i < count; i++) {
			    var group = chart.svg.group(),
				    isTargetString  = _.typeCheck("string", brush.target[i]),
				    isTargetVisible = isTargetString || brush.target[i].visible,
				    target          = brush.target[i],
				    targetField     = isTargetString ? target : target.field,
				    text            = isTargetString ? targetField : target.name || targetField ,
				    color           = chart.color(i, brush.colors);

			    // 컬러 인덱스 설정
			    colorIndex[targetField] = color;

			    // 타겟 별 포맷 설정
			    if(_.typeCheck("function", widget.format)) {
				    text = this.format(targetField, target);
			    }
			    ...
			    if(widget.filter) {
				    ...
				    changeLegend(target, group, isTargetVisible);

				    (function(currentTarget, key, element) {
					    element.attr({
						    cursor: "pointer"
					    });

					    element.on("click", function(e) {
						    columns[brush.index][key] = !columns[brush.index][key];
						    changeLegend(currentTarget, element, columns[brush.index][key]);
						    changeTargetOption((widget.brushSync) ? getBrushAll() : [ brush ]);
						    chart.render(true);
					    });
				    })(target, targetField, group);
			    } else {
				    var size = chart.theme("legendFontSize");

				    if(widget.icon != null) {
					    var icon = _.typeCheck("function", widget.icon) ? widget.icon.apply(chart, [ targetField ]) : widget.icon;
		    ...
		    function changeLegend (target, element, isVisible) {
			    var isTargetString = (typeof target === "string"),
				    key = isTargetString ? target : target.field;
			    ...
			    if(isVisible) {
				    ...
				    if (!isTargetString) { target.visible = true; }
			    } else {
				    ...
				    if (!isTargetString) { target.visible = false; }
			    }
		    }
