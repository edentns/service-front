(function () {
	...
	angular
		 ...
		 * 서비스 접수 리스트 Biz
		 */
		.service('SV.01svServRecListBiz', ['UtilSvc', 'SvCommonSvc', 'SvListPageVO', 'SV.01svServRecSvc', 'SY.codeSvc', 'ModalSvc', 'APP_CODE', '$q', '$timeout',
			function (UtilSvc, SvCommonSvc, SvListPageVO, SvServRecSvc, SyCodeSvc, ModalSvc, APP_CODE, $q, $timeout) {
				 ...
				 * @returns {*}
				 */
				self.initLoad = function (search, servRec) {
					var self = this,
						defer = $q.defer();
					...
					$timeout(function () {
						...
						compositeParam.procedureParam = servRec.grid.procedureParam;
						...
						self.getCodeAndData(compositeParam).then(function (res) {
							...
							servRec.grid.data = res[9].data.results[0];

							self.setDataAndParam(search, servRec);
				...
				self.setDataAndParam = function (search, servRec) {
					self._setParam(search);
					self._setGridColumn(servRec);
				};
				 ...
				 * @private
				 */
				self._setGridColumn = function (servRec) {
					UtilSvc.grid.initColumnSetting(servRec);
				};
				 ...
				 */
				self.modalSetColumn = function (servRec) {
					var column = servRec.column,
						grid   = servRec.grid,
