(function () {
	 ...
	 * 서비스 접수 리스트 Biz
	 */
		.service('SV.03svServPfmListBiz', ['UtilSvc', 'SvCommonSvc', 'SvListPageVO', 'SV.03svServPfmSvc', 'SY.codeSvc', 'ModalSvc', 'APP_CODE', '$q', '$timeout',
			function (UtilSvc, SvCommonSvc, SvListPageVO, SvServPfmSvc, SyCodeSvc, ModalSvc, APP_CODE, $q, $timeout) {
				 ...
				 * @returns {*}
				 */
				self.initLoad = function (search, servPfm) {
					var self = this,
						defer = $q.defer();
					...
					$timeout(function () {
						...
						compositeParam.procedureParam = servPfm.grid.procedureParam;
						...
						self.getCodeAndData(compositeParam).then(function (res) {
							...
							servPfm.grid.data = res[9].data.results[0];

							self.setDataAndParam(search, servPfm);
				...
				self.getCodeAndData = function (param) {
					...
					return $q.all([
						...
						UtilSvc.getList(param)                                   // TODO: 계획리스트를 가져온다.
					]);
				};
				...
				/**
				 * grid column을 설정한다.
				 * @param {object} servPfm 서비스접수 Model
				 * @private
				 */
				self._setGridColumn = function (servPfm) {
					UtilSvc.grid.initColumnSetting(servPfm);
				};
				...
				/**
				 * Grid Column을 설정한다.
				 * @param {object} servPfm 서비스수행
				 */
				self.modalSetColumn = function (servPfm) {
					var column = servPfm.column,
						grid   = servPfm.grid,
