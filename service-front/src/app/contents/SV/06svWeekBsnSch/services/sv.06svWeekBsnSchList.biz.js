(function () {
	...
	angular
		.module("SV.06svWeekBsnSch.service")
		/**
		 * 서비스 분석 업무현황 리스트 Biz
		 */
		.service('SV.06svWeekBsnSchListBiz', ['UtilSvc', 'SvCommonSvc', 'SvListPageVO', 'SY.codeSvc', 'ModalSvc', 'APP_CODE', '$q', '$timeout',
			function (UtilSvc, SvCommonSvc, SvListPageVO, SyCodeSvc, ModalSvc, APP_CODE, $q, $timeout) {
				...
				self.initLoad = function (searchObj, gridObj) {
					...
					$timeout(function () {
						...
						self.getCodeAndData(compositeParam).then(function (res) {
							...
							searchObj.code.servTpCdList     = res[4].data;
				...
				/**
				 * grid column을 설정한다.
				 * @param {object} svWeekBsnSch 서비스접수 Model
				 * @private
				 */
				...
				/**
				 * Grid Column을 설정한다.
				 * @param {object} svWeekBsnSch 서비스접수
				 */
